...
class Monitor:
    ...
    """
    def __init__(self, cmpt, client, printer):
    ...
    async def run(self):
        ...
        while True:
            try:
                recently_changed = self.client.recently_changed_packages_activity_list()
            except self.client.exception:
            ...
            await self.watch_changed_packages(
                recently_changed, # list of dicts returned from client
                list_checked_packages, # list that is added to as program runs
                self.client.package_show # data sent to get_resources
            )
...
async def monitor_for_changes(cmpt, client, printer):
    """
    creates Monitor object
    """
    monitor = Monitor(cmpt, client, printer)
    await monitor.run()
